  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        ...
        toast({ title: '등록 완료', description: '새로운 공지사항이 등록되었습니다.' })
      }
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                {editingId ? '내용을 수정합니다.' : '전체 직원에게 전달할 공지사항을 작성하세요.'}
              </DialogDescription>
            </DialogHeader>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {announcements.length > 0 
                ? Math.round(announcements.reduce((acc, a) => acc + a.views, 0) / announcements.length)
                : 0}
              회
            </div>
          </CardContent>
